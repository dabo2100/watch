  ...
  return (
    <div className="w-full py-12 bg-white text-center relative">
      <div className="lg:flex lg:justify-between items-center">
        ...
        <div className="flex justify-center gap-6 mb-8 text-gray-500 flex-wrap">
          {Object.keys(productTabs).map((el) => (
            <button key={el} onClick={() => setActiveTab(el)} className={`pb-1 transition-all ${activeTab === el ? 'text-orange-500 border-b-2 border-orange-500 font-medium' : 'hover:text-orange-400'}`}>
              {el}
            </button>
          ))}
        </div>
      ...
      <div
        ...
        sm:grid-cols-2 
        md:grid-cols-4
        lg:${activeTab === 'On Sell' ? 'grid-cols-4' : 'grid-cols-1 justify-items-center'}`}
      >
        {products.map((el, i) => (
          <div key={el.id} className="relative group rounded-2xl hover:shadow-md transition-all duration-300 p-4 bg-white">
            <img src={el.img} alt={el.name} className="w-full h-52 object-contain mx-auto mb-3" />

            <h3 className="text-gray-800 font-medium text-base">{el.name}</h3>
            <div className="mt-1">
              {el.oldPrice && <span className="text-gray-400 line-through mr-2 text-sm">{el.oldPrice}</span>}
              <span className="text-black font-semibold text-sm">{el.price}</span>
            </div>

            {activeTab === 'On Sell' && i !== products.length - 1 && <div className=" lg:block absolute right-0 top-1/4 bottom-1/4 w-[1px] bg-gray-200"></div>}
          </div>
        ))}

        {activeTab !== 'On Sell' && <div className="hidden lg:block absolute right-0 top-0 bottom-0 w-[1px] bg-gray-200"></div>}
      </div>
    </div>
  );
}
